...
public abstract class AbstractConfigurationLoader {
	...
	private final static Logger LOG = Logger.getLogger(AbstractConfigurationLoader.class); 
	...
	private synchronized Map load(){
		...
		try {
            ...
            while (ite.hasNext()) {
                ...
                catch  (SAXException e) { // JSO-15 Catch only Exception
                	LOG.error("Parsing of file " + url.toString() + " failed! Parsing still continues.", e);
                }
                catch (IOException e) { // JSO-15 Catch only Exception
