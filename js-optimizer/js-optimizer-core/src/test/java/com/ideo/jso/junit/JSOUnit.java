...
public abstract class JSOUnit extends TestCase {
	 ...
	 */
	protected Properties initJSO(String jsoFilePath) 
...
//		System.out.println("   " + properties.toString());
     ...
     * @throws IOException
     */
    protected void checkConfigFileName(String configFileName, int occurences, Properties properties) 
    throws IOException {
		...
		assertTrue("jso.configFileName contains " + configFileName, 
						properties.getProperty("jso.configFileName").trim().indexOf(configFileName.trim()) >= 0);
	...
	protected void checkCssFilesList(Group group, String[] files) {
		...
		if (files == null) {
			...
			// CSS files list contains ordered file name.
			...
			for (int i = 0; i < files.length; i++) {
				...
				assertEquals("Group " + name + "'s css files list contains file " + file +" at position " + i + ".", 
							files[i], file );
			}
	...
	protected void checkJsFilesList(Group group, String[] files) {
		...
		if (files == null) {
			...
			// JS files list contains ordered file name.
			...
			for (int i = 0; i < files.length; i++) {
				...
				String file = (String) iterator.next();
				assertEquals("Group " + name + "'s js files list contains file " + file +" at position " + i + ".", 
							files[i], file );
			}
